...
As we see above for the overfitted model, while the overfit gap in accuracy scores is only around 2%, cummulative gains drop by 50%. 
...
However, this leaves us with several open-ended questions i.e.: can we now calcualte shapley values indirectly, using previously presented effecient technique? Surely the max function breaks the addivity of uplift function, but maybe it is possible using some other method? 
